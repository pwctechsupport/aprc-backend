        ...
        if args[:publish] === true
          ...
          if policy.references.present?
            ...
            ref.each do |r|
              ...
              nama = namu.join(", ")
              Notification.send_notification_to_all(admin ,"#{polisi.title} with #{r.name} reference has the same references with #{nama}.","Policy with the same reference",policy, current_user&.id, "same_reference" )  
            end
          end
        else
          policy_draft.revert!
