      ...
      if current_user.present? && current_user.has_role?(:admin_reviewer)
        ...
        if args[:publish] === true
          if control.user_reviewer_id.present? && (control.user_reviewer_id != current_user.id)
            ...
            Notification.send_notification(admin_prep, "Control Draft with owner #{control&.control_owner} Approved", control&.name,control, current_user&.id, "request_draft_approved")
          else
            control_draft.publish!
            control.update(user_reviewer_id: current_user.id)
            control.update(status: "release")
          ...
          if control.user_reviewer_id.present? && (control.user_reviewer_id != current_user.id)
            ...
            Notification.send_notification(admin_prep, "Control Draft with owner #{control&.control_owner} Rejected", control&.name,control, current_user&.id, "request_draft_rejected")
